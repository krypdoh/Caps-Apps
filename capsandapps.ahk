...
; Program:  Caps&Apps.ahk
; Version:  0.1
; Author:   Paul R. Charovkine  
; Website:  github.com/krypdoh/Caps&Apps
; License:  MIT
; Date:     2026-05-20

; === CapsLock Auto-Enable for Specific Applications ===
; Enables CAPS LOCK when the user is actively focused on a configured .exe program.
; Disables CAPS LOCK when switching away from those programs.

; --- Configuration ---
global AppList := []
global ConfigFile := A_ScriptDir "\capsandapps.ini"
global MonitorTimer := 0
global CapsWasForced := false
...
BuildTrayMenu() {
    ...
    TraySetIcon(A_ScriptDir "\capsandapps.ico")
    A_IconTip := "CapsLock Auto-Enable"
}
...
CheckActiveWindow() {
    ...
    if (shouldBeOn && !GetKeyState("CapsLock", "T")) {
        SetCapsLockState("On")
        CapsWasForced := true
    } else if (!shouldBeOn && CapsWasForced && GetKeyState("CapsLock", "T")) {
...
GetUniqueProcesses() {
    ...
    for proc in ComObjGet("winmgmts:").ExecQuery("SELECT Name, ProcessId FROM Win32_Process") {
        ...
        ; Try to get a window title for this process
        title := ""
        try {
...
AddApplication(exeName) {
    ...
    SaveConfig()
    MsgBox(exeName " has been added.`nCapsLock will auto-enable when this program is active.", "Program Added", "Iconi")
}
...
ShowManagedList() {
    ...
    myGui.SetFont("s10")
    myGui.Add("Text", "w350", "Programs that trigger CapsLock (select to remove):")
...
RemoveSelected(myGui, lb) {
    selected := lb.Value  ; array of selected indices for Multi listbox
...
; --- About ---
ShowAbout() {
    aboutGui := Gui("+Owner +AlwaysOnTop", "About Caps&&Apps")
    aboutGui.SetFont("s10")
    aboutGui.Add("Text", "w360", "Program:  Caps&Apps")
    aboutGui.Add("Text", "w360", "Version:  0.1")
    aboutGui.Add("Text", "w360", "Author:   Paul R. Charovkine")
    aboutGui.Add("Link", "w360", 'Website:  <a href="https://github.com/krypdoh/Caps-Apps">github.com/krypdoh/Caps-Apps</a>')
    aboutGui.Add("Text", "w360", "License:  MIT")
    aboutGui.Add("Text", "w360", "Date:     2026-05-20")
